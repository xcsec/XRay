...
contract SafeERC20BridgeWithPool is AbstractBridge, Mutex {
    uint256 constant DECIMALS = 6;

    mapping(uint128 => mapping(address => BindingInfo)) public bindings;
    mapping(address => uint256) public fees;
    mapping(address => uint256) public balances;
    mapping(uint128 => mapping(address => BindingInfo)) public poolBindings;
    ...
    function lockTokensInPool(
        ...
        require(
            poolBindings[executionChainId_][token_].enabled,
            "token is disabled"
        );
        require(
            amount_ >= poolBindings[executionChainId_][token_].minAmount,
            "less than min amount"
        );
        uint256 fee = calculatePoolFee_(executionChainId_, token_, amount_);
        require(amount_ > fee, "fee more than amount");
        unchecked {
            amount_ = amount_ - fee;
        }
        ...
        IMint(adapter).mintTokens(
            executionChainId_,
            poolBindings[executionChainId_][token_].executionAsset,
            amount_ / divider,
            recipient_,
            gaslessReward_ / divider,
            referrer_,
    ...
    function calculatePoolFee_(
        uint16 executionChainId_,
        address token_,
        uint256 amount_
    ) private view returns (uint256) {
        uint128 percent = amount_ >
            poolBindings[executionChainId_][token_].thresholdFee
            ? poolBindings[executionChainId_][token_].afterPercentFee
            : poolBindings[executionChainId_][token_].beforePercentFee;

        return
            poolBindings[executionChainId_][token_].minFee +
            (amount_ * percent) /
            PERCENT_FACTOR;
    }
    ...
    function updatePoolBindingInfo(
        uint16 executionChainId_,
        address token_,
        string calldata executionAsset_,
        uint256 minAmount_,
        ...
        poolBindings[executionChainId_][token_] = BindingInfo(
            executionAsset_,
            minAmount_,
            minFee_,
            thresholdFee_,
