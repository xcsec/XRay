...
class BypassingCrosschainMessageCheckRelayer(AbstractDetector):
    """
    Missing events for critical contract parameters set by owners and used in access control
    """

    ARGUMENT = "bypassing-crosschain-message-check-relayer"
    HELP = "Missing crosschain data check on destination chain in a relayer-based cross-chain bridge"
    ...
    WIKI = ""
    WIKI_TITLE = "incorrect validation on crosschain message in relayer-based cross-chain bridges"
    WIKI_DESCRIPTION = "incorrect validation on crosschain message in relayer-based cross-chain bridges"
    dependency_relation = {}
...
"""
    ...
    WIKI_RECOMMENDATION = "bypassing-crosschain-message-check-relayer"
    ...
    def _detect_missing_crosschain_data_check(
        ...
        for function in contract.functions_entry_points:
            ...
            for call in potential_process_call:
                for dominator in call.dominators:
                    if dominator.is_conditional:
                        for call in set(dominator.internal_calls) & set(dominator.high_level_calls) & set(dominator.low_level_calls):
                            dominator.ir += call.irs
                    for ir in dominator.irs:
                        bytes32_flag = False
                        hash_flag = False
                        if isinstance(ir, HighLevelCall) or isinstance(ir, InternalCall):
                            if contains_any(ir.function.full_name, merklepathsig) or contains_any(ir.function.full_name, merklerootsig):
                                bytes32_flag = True
                            if any(hash_function in call_name for hash_function in HASH_FUNCTIONS for call_name in [call.name for call in ir.function.internal_calls]
):
                                hash_flag = True
                        if bytes32_flag and hash_flag:
                            for arg in ir.arguments:
                                if  isinstance(arg.type, ElementaryType) and arg.type.name == "bytes32" and isinstance(arg, LocalVariable):
                                    if is_tainted(arg, function):
                                        missing_check_process_call[call] = False
                                        # print("Incorrect Merkle Proof Verification in op-based crosschain")
                                if isinstance(arg.type, ElementaryType) and arg.type.name == "bytes32" and isinstance(arg, StateVariable):
                                    if is_tainted(arg, contract):
                                        missing_check_process_call[call] = False
                                        # print("Incorrect Merkle Proof Verification in relayer-based crosschain")
                #
                # if call not in missing_check_process_call:
                #     missing_check_process_call[call] = False
                # for dominator in call.dominators:
    ...
    def _detect(self) -> List[Output]:
        ...
        for contract in self.compilation_unit.contracts_derived:
            missing_crosschain_data_checks = self._detect_missing_crosschain_data_check(contract,
                                                                                        ...
                                                                                        self.CROSSCHAINRECEIVEEVENTLIST, self.MerklePathSig, self.MerklePathSig, self.HASH_FUNCTIONS)
            for function in missing_crosschain_data_checks:
                info: DETECTOR_INFO = ["Missing Crosschain Data Check was found in a relay bridge ", function, "\n"]
