...
contract CoinBridge is AbstractBridge, Mutex {
    ...
    function releaseTokens(
        ...
        if (gaslessReward_ > 0 && recipient_ != tx.origin) {
            (bool recipientTransferStatus, ) = recipient_.call{
                value: amount_ - gaslessReward_
            }("");
            require(recipientTransferStatus, "recipient_ may have reverted");
            (bool relayerTransferStatus, ) = payable(tx.origin).call{
                value: gaslessReward_
            }("");
            require(relayerTransferStatus, "relayer_ may have reverted");
        } else {
            (bool recipientTransferStatus, ) = recipient_.call{value: amount_}(
                ""
            );
            require(recipientTransferStatus, "recipient_ may have reverted");
        }
