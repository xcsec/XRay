...
class CrosschainReplay(AbstractDetector):
    """
    Missing events for critical contract parameters set by owners and used in access control
    """

    ARGUMENT = "bridged-message-replay"
    HELP = "Missing crosschain message replay detect on destination chain"
    ...
    WIKI = ""
    WIKI_TITLE = ("Bridged Message Replay")
    WIKI_DESCRIPTION = "Bridged Message Replay"
    dependency_relation = {}
...
"""
    ...
    def _detect_missing_crosschain_data_check(
        ...
        for function in contract.functions_entry_points:
            ...
            has_storage_var = False
            update_storage_var = False
            ...
            for node in function.nodes:
                ...
                for ir in node.irs + slithir_operation:
                    if isinstance(ir, HighLevelCall) or isinstance(ir, LowLevelCall):
                        ...
                        if not len(node.state_variables_written) == 0:
                            update_storage_var = True
            ...
            for key in missing_check_process_call.keys():
                if not missing_check_process_call[key]:
                    results.append(function)
            if not has_storage_var or not update_storage_var:
                results.append(function)
    ...
    def _detect(self) -> List[Output]:
        ...
        for contract in self.compilation_unit.contracts_derived:
            ...
            for function in missing_crosschain_data_checks:
                info: DETECTOR_INFO = ["Cross Message Replay was Found ", function, "\n"]
