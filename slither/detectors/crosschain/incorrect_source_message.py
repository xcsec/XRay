...
class IncorrectEvent(AbstractDetector):
    ...
    ARGUMENT = "incorrect-source-message"
    HELP = "An incorrect source message emitted in crosschain bridge"
            ...
            for node in function.nodes:
                ...
                for ir in node.irs + slithir_operations:
                    ...
                    if isinstance(ir, HighLevelCall) or isinstance(ir, LowLevelCall):
                        if not is_tainted(ir.destination,contract):
                            external_call.append(node)
                        if isinstance(ir, HighLevelCall) and ir.function.solidity_signature in ["transfer(address,uint256)", "transferFrom(address,address,uint256)"]:
                            results.append(function)
