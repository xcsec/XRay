...
contract ERC20Bridge is AbstractBridge, Mutex {
    ...
    function releaseTokens(
        ...
        if (gaslessReward_ > 0 && recipient_ != tx.origin) {
            assert(
                token.transfer(recipient_, amount_ - gaslessReward_) &&
                    token.transfer(tx.origin, gaslessReward_)
            );
        } else {
            assert(token.transfer(recipient_, amount_));
        }
